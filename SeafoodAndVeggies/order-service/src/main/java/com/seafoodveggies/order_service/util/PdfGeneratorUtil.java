...
import com.seafoodveggies.order_service.dto.OrderItemResponseDTO;
...
public class PdfGeneratorUtil {
    ...
    public static byte[] generateOrderInvoice(OrderResponseDTO order) throws Exception {
        ...
        shippingDetails.addCell(new Phrase(addr.fullName(), normalFont));
        shippingDetails.addCell(new Phrase(addr.flatOrHouseNumber() + ", " + addr.addressLine1(), normalFont));
        if (addr.addressLine2() != null && !addr.addressLine2().isEmpty()) {
            shippingDetails.addCell(new Phrase(addr.addressLine2(), normalFont));
        }
        shippingDetails.addCell(new Phrase(addr.city() + ", " + addr.state() + " - " + addr.postalCode(), normalFont));
        shippingDetails.addCell(new Phrase(addr.country(), normalFont));
